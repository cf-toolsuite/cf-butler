...
public class DatabaseCreator implements ApplicationRunner {
	...
	public void run(ApplicationArguments args) throws Exception {
		try (Connection c = database.connection().blockingGet()) {
			...
			createApplicationPoliciesTable(c);
			createServiceInstancePoliciesTable(c);
	...
	protected void createApplicationPoliciesTable(Connection c) throws SQLException {
		c.prepareStatement("create table application_policies ( description varchar(1000), state varchar(25), from_datetime timestamp, from_duration varchar(25), unbind_services boolean, delete_services boolean )")
			.execute();
		
	}
	
	protected void createServiceInstancePoliciesTable(Connection c) throws SQLException {
		c.prepareStatement("create table service_instance_policies ( description varchar(1000), from_datetime timestamp, from_duration varchar(25) )")
			.execute();
	}
